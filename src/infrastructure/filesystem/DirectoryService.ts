...
export class DirectoryService {
	 ...
	 * @param integration Diagnostic integration (eslint, typescript, vitest)
	 * @returns Path to .omnyreporter/{integration} directory
	 */
	public getInstrumentDirectory(integration: IntegrationName): string {
		return resolve(this.getAppDirectory(), integration);
	}

	/**
	 * Get errors directory for specific instrument
	 * @param integration Diagnostic integration
	 * @returns Path to .omnyreporter/{integration}/errors directory
	 */
	public getInstrumentErrorsDirectory(integration: IntegrationName): string {
		return resolve(this.getInstrumentDirectory(integration), ERRORS_DIR);
	}
	...
	public async clearInstrumentErrors(integration: IntegrationName): Promise<void> {
		const errorsDir = this.getInstrumentErrorsDirectory(integration);
	...
	public async clearAllErrors(): Promise<void> {
		const integrations: IntegrationName[] = [
			IntegrationName.ESLint,
			IntegrationName.TypeScript,
			IntegrationName.Vitest,
		];

		for (const integration of integrations) {
			await this.clearInstrumentErrors(integration);
		}
	}
}
